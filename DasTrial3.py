...
def main():
    ...
    # Step 4: Set up Dask client on the main node
    # Use the scheduler's IP address for the client to connect to
    logger.info(f"Trying to connect the workers to the main node (scheduler) at {SCHEDULER_IP}:{SCHEDULER_PORT}")
    client = Client(f'{SCHEDULER_IP}:{SCHEDULER_PORT}')  # Main node IP (scheduler)
    logger.info(f"Dask Dashboard available at: {client.dashboard_link}")

    # Step 5: Perform continuous computation (running forever until manually canceled)
