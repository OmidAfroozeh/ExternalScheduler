...
async def main():
    ...
    # Step 3: Start the external SchedulerService (Flask app) **last**
    def is_flask_app_running():
        """Check if the Flask app (SchedulerService) is already running."""
    ...
    try:
        if not is_flask_app_running():
            ...
            logger.info("Flask app is already running. No need to start again.")
    except Exception as e:
